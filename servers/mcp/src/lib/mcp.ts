...
function validateOrigin(request: Request, env: Env): boolean {
  ...
  // Allow our domains and known AI clients
  const allowedOrigins = [
    'https://mcp.trendingsociety.com',
    'https://trendingsociety.com',
    'https://dashboard.trendingsociety.com',
    'https://claude.ai',
 ...
 */
function validateProtocolVersion(request: Request): { valid: boolean; version: string } {
 ...
 * Validate session ID
 */
function validateSession(request: Request): { valid: boolean; sessionId: string | null } {
...
export async function handleMcpMessage(
  ...
  env: Env,
  toolHandler: (tool: string, params: Record<string, unknown>) => Promise<unknown>,
  ...
  switch (method) {
    ...
    case 'tools/call':
      ...
      try {
        const result = await toolHandler(toolParams.name, toolParams.arguments || {});
        return {
          response: jsonRpcResponse(id, {
            content: [
              {
        ...
        return { response: jsonRpcError(id, -32603, `Tool execution failed: ${err}`) };
      }
...
export function createMcpStreamHandler(
  ...
  return (request: Request): Response => {
    ...
    if (!validateOrigin(request, env)) {
      return new Response(JSON.stringify(jsonRpcError(null, -32600, 'Origin not allowed')), {
        status: 403,
...
export async function handleMcpMessagePost(
  ...
  if (!protocolValidation.valid) {
    return new Response(JSON.stringify(jsonRpcError(null, -32600, `Unsupported protocol version: ${protocolValidation.version}`)), {
      status: 400,
      headers: { 'Content-Type': 'application/json' }
    });
  }
  ...
  try {
    ...
    const { response, sessionId } = await handleMcpMessage(body, env, toolHandler, sessionValidation.sessionId);
 ...
 */
export async function handleMcpDelete(request: Request, env: Env): Promise<Response> {
  const sessionId = request.headers.get('MCP-Session-Id');
